...
import { VmState } from '@/types/vmStateEnum';
...
async function toggle(): Promise<void> {
  try {
    ...
    toast.add({ severity: 'error', summary: `Failed to toggle VM: ${(err as Error)?.message}`, group: 'tc', life: 5000 });
  }
};
      ...
      <template #content>
        <div class="space-y-6">
          ...
          <div class="pb-4 border-b border-surface-200 dark:border-gray-700">
            <p class="text-xs uppercase tracking-widest text-primary">Plex Infrastructure</p>
          ...
          <div class="flex flex-col gap-8">
            ...
            <section class="flex items-center justify-center">
              <div
                ...
                <svg
                  :class="['h-72 w-72 status-wrapper', { 'click-burst': isClicking }]" viewBox="0 0 100 100"
                  :style="{ '--status-color': statusColor }" xmlns="http://www.w3.org/2000/svg" @click="buttonPress()">
                  <circle
                    cx="50" cy="50" r="35" :fill="statusColor"
                    :class="['center-dot', { 'cursor-pointer': canToggle }]" />
                  <text
                    x="50" y="52" text-anchor="middle" dominant-baseline="middle" class="status-text tracking-wide"
                    fill="currentColor">
                    {{ statusText }}
                  </text>
                </svg>
            ...
            <section class="space-y-4">
              <div class="flex flex-col gap-3 items-center justify-center w-full">
                <Button
                  :label="toggleLabel" :loading="isToggleLoading" :disabled="!canToggle"
                  :severity="isOnline ? 'danger' : 'success'" class="flex-1 w-9/10 sm:w-md" @click="toggle" />
                <Button
                  label="Refresh" icon="pi pi-refresh" outlined class="flex-1 w-9/10 sm:w-md"
                  @click="clickRefresh()" />
              </div>
            </section>
            ...
            <section class="flex justify-start pt-2">
              <div class="flex items-center gap-3 rounded-lg bg-surface-50 dark:bg-surface-800 px-4 py-3">
                <i
                  :class="[statusIcon, isLoading ? 'pi-spin' : '', 'text-2xl text-primary']"
